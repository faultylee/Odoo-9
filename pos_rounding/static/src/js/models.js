...
exports.PosModel = Backbone.Model.extend({
    ...
    models: [
    {
        ...
        model:  'res.company',
        fields: [ 'currency_id', 'email', 'website', 'company_registry', 'street', 'street2', 'city', 'zip', 'state_id', 'vat', 'name', 'phone', 'partner_id' , 'country_id', 'tax_calculation_rounding_method'],
...
exports.Order = Backbone.Model.extend({
    ...
    export_as_JSON: function() {
        ...
        return {
            ...
            amount_round_off: this.get_round_off(),
            lines: orderLines,
    ...
    export_for_printing: function(){
        ...
        var receipt = {
            ...
            total_round_off: this.get_round_off(),
    ...
    get_total_with_tax: function() {
        var total_before_round = this.get_total_without_tax() + this.get_total_tax()
        var str =total_before_round.toString();
        var sp = str.split('.')[1]
        var digits = (""+sp).split("");
        var total=0;
        if (digits[1] == 1){
            total = total_before_round - 0.01;
        }
        else if (digits[1] == 2){
            total = total_before_round - 0.02;
        }
        else if (digits[1] == 3){
            total = total_before_round - 0.03;
        }
        else if (digits[1] == 4){
            total = total_before_round + 0.01;
        }
    ...
    get_tax_details: function(){
        ...
        for(var id in details){
            if(details.hasOwnProperty(id)){
                fulldetails.push({amount: details[id], tax: this.pos.taxes_by_id[id], name: this.pos.taxes_by_id[id].name, base: detailsTaxBase[id]});
            }
        }
