...
class pos_order(osv.osv):
    ...
    def _amount_all(self, cr, uid, ids, name, args, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            total_before_round_off = res[order.id]['amount_tax'] + amount_untaxed
            amount_round_off= 0.0
            if int(str(total_before_round_off).split('.')[1]) != 0:
                if int(str(total_before_round_off).split('.')[1][1:]) in (1,6):
                    amount_round_off = -0.01
                elif int(str(total_before_round_off).split('.')[1][1:]) in (2,7):
                    amount_round_off = -0.02
                elif int(str(total_before_round_off).split('.')[1][1:]) in (3,8):
                    amount_round_off = 0.02
                elif int(str(total_before_round_off).split('.')[1][1:]) in (4,9):
                    amount_round_off = 0.01
                else:
                    amount_round_off = 0.00
            res[order.id]['amount_round_off'] = amount_round_off
            res[order.id]['amount_total'] = res[order.id]['amount_tax'] + amount_untaxed  + amount_round_off
        return res
    ...
    _columns = {
        ...
        'amount_round_off': fields.function(_amount_all, string='Round Off', digits=0,  multi='all'),
    ...
    def _create_account_move_line(self, cr, uid, ids, session=None, move_id=None, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            if order.amount_round_off != 0.0:
            # Create a move for the round off
                move_line =  {
                            'name': name,
                            'quantity': 1,
                            'product_id': False,
                            'account_id': income_account,
                            'analytic_account_id': self._prepare_analytic_account(cr, uid, line, context=context),
                            'credit': ((order.amount_round_off>0) and order.amount_round_off) or 0.0,
                            'debit': ((order.amount_round_off<0) and -order.amount_round_off) or 0.0,
                            'partner_id': order.partner_id and self.pool.get("res.partner")._find_accounting_partner(order.partner_id).id or False
                        }
